...
onMount(async () => {
  ...
  mapHandle = createThemedMap({
    ...
    managerOptions: {
      ...
      // the vessel itself. The optional vessel wind rose still renders in the vessel band, but is
      // deliberately not locked here so the Layers panel can turn its default-off state on.
